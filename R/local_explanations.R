...
#' @param magnitude If TRUE, only return explanations.
...
#'   }
#' @export
get_local_explanations_gradient <- function(model, input_data, num_samples = 1,
                                            magnitude = TRUE,
  ...
  inds <- torch::torch_nonzero(input_data == 0)[, 2] #find index of 0s
...
#' @description Plots the contribution of each covariate,
#' and the prediction, with error bars.
...
#' effect and does not return a value.
plot_local_explanations_gradient <- function(model, input_data, num_samples,
                                             device = "cpu", save_svg = NULL) {
  outputs <- get_local_explanations_gradient(model = model,
  ...
  for (cls in 1:model$sizes[length(model$sizes)]) {
    ...
    pred_median <- pred_quantiles[, cls][2] #add the median of the prediction
    ...
    plt <- ggplot2::ggplot(data <- data, ggplot2::aes(x = factor(name, levels = name),
      ...
      ggplot2::geom_errorbar(ggplot2::aes(x = name, ymin = min, ymax = max),
                             width = 0.6, colour = "black", alpha = 0.9, size = 0.5) +
      ggplot2::xlab("") + ggplot2::ylab("Contribution") + ggplot2::ggtitle(
        "Local explanation, with 95% empirical confidence bars")
    print(plt)
    if (!is.null(save_svg)) {
      new_file <- file.path(
        dirname(save_svg),
