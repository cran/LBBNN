#' @title Generate prior inclusion probabilities for the weights of a
#' LBBNN layer (linear or convolutional).
#' @description A function to generate prior probability values for
#' each weight in an LBBNN layer.
#' The same probability is applied to all weights within a layer.
#' @param x A number between 0 and 1.
#' @return a numeric to be added to either (out_shape,in_shape)
#' in case of linear layers, or (out_channels,in_channels,kernel0,kernel1)
#' in case of convolutional layers.
#' @keywords internal
...
#' @title Generate prior standard deviation for weights and biases of either
#' linear or convolutional layers.
...
#' @description Controls the initial density of the network, an important tuning
...
#' @param prior_inclusion numeric scalar, prior inclusion probability
...
#'   - \code{forward(input, MPM = FALSE)}: Computes activation
#'   (using the LRT at training time) of a batch of inputs.
#'   - \code{kl_div()}: Computes the KL-divergence.
#'   - \code{sample_z()}: Samples from the flow if \code{flow = TRUE},
...
lbbnn_linear <- torch::nn_module(
  ...
  initialize = function(
    ...
    self$weight_mean <- torch::nn_parameter(torch::torch_empty(out_features,
                                                               in_features,
                                                               device = device))
    self$weight_rho <- torch::nn_parameter(torch::torch_empty(out_features,
                                                              in_features,
                                                              device = device))
    self$weight_sigma <- torch::torch_empty(out_features, in_features,
                                            device = device)

    #bias variational parameters
    self$bias_mean <- torch::nn_parameter(torch::torch_empty(out_features,
                                                             device = device))
    self$bias_rho <- torch::nn_parameter(torch::torch_empty(out_features,
                                                            device = device))
    self$bias_sigma <- torch::torch_empty(out_features, device = device)

    #inclusion variational parameters
    self$lambda_l <- torch::nn_parameter(torch::torch_empty(out_features,
                                                            in_features,
                                                            device = device))
    self$alpha <- torch::torch_empty(out_features, in_features, device = device)
    self$alpha_active_path <- torch::torch_empty(out_features, in_features,
                                                 device = device)

    #define priors
    self$alpha_prior <- torch::torch_zeros(out_features, in_features, device =
                                             device) + alpha_prior(
                                             prior_inclusion)

    if (self$bias_inclusion_prob) {
    self$bias_lambda_l <- torch::nn_parameter(torch::torch_empty(out_features,
                                                                 device = device
                                                                 ))
    self$bias_alpha <- torch::torch_empty(out_features, device = device)

    #define priors for bias.
    self$bias_alpha_prior <- (torch::torch_zeros(out_features, device = device)
                              + alpha_prior(prior_inclusion))
    }

    #standard normal prior on the weights and biases
    self$weight_mean_prior <- torch::torch_zeros(out_features, in_features,
                                                 device = device)
    self$weight_sigma_prior <- (torch::torch_zeros(out_features, in_features,
                                                  device = device)
                                + std_prior(standard_prior))
    self$bias_mean_prior <- torch::torch_zeros(out_features, device = device)
    self$bias_sigma_prior <- (torch::torch_zeros(out_features, device = device)
                              + std_prior(standard_prior))

    #flow parameters
    if (self$flow) {
      self$q0_mean <- torch::nn_parameter(torch::torch_empty(in_features,
                                                             device = device))
      self$q0_logvar <- torch::nn_parameter(torch::torch_empty(in_features,
                                                               device = device))
      self$c1 <- torch::nn_parameter(torch::torch_empty(in_features,
                                                        device = device))
      self$b1 <- torch::nn_parameter(torch::torch_empty(in_features,
                                                        device = device))
      self$b2 <- torch::nn_parameter(torch::torch_empty(in_features,
                                                        device = device))

      self$RNVP_flow <- normalizing_flow(c(in_features, hidden_dims), "RNVP",
                                         self$num_transforms)
  ...
  reset_parameters = function() {
    ...
    torch::nn_init_uniform_(self$lambda_l, self$density_init[1],
                            self$density_init[2])
    if (self$flow) {
  ...
  sample_z = function() {
    ...
    self$z <- self$q0_mean + q0_std * epsilon_z #the initial random var
    out <- self$RNVP_flow(self$z) #returns z_k and the log determinant
    return(out)
  }
  ,
  forward = function(input, MPM = FALSE) {
    ...
    if (! MPM) {#compute  mean and the variance of the activations using the LRT
      e_w <- self$weight_mean * self$alpha * self$z_k
      var_w <- self$alpha * (self$weight_sigma^2 + (1 - self$alpha) *
                               self$weight_mean^2 * self$z_k^2)
      if (self$bias_inclusion_prob) {
        e_b_alpha <- self$bias_mean * self$bias_alpha
        var_b_alpha <- self$bias_alpha * (self$bias_sigma^2 +
                                      (1 - self$bias_alpha) * self$bias_mean^2)
      } else {
  ...
  kl_div = function() {
    ...
    if (self$flow) {
      ...
      log_rb <- torch::torch_sum(-0.5 *
                                  torch::torch_log(pi)$to(device = self$device)
                                 - 0.5 * log_var_r
                                - 0.5 * ((z_b - mean_r) ** 2
                                         / torch::torch_exp(log_var_r)))
    ...
    if (self$bias_inclusion_prob) {
      kl_bias <- torch::torch_sum(
        self$bias_alpha * (
          ...
          + (self$bias_sigma^2 + (self$bias_mean - self$bias_mean_prior)^2)
          / (2 * self$bias_sigma_prior^2)
        )
        + (1 - self$bias_alpha) * torch::torch_log((1 - self$bias_alpha)
                                                 / (1 - self$bias_alpha_prior)
                                                 + 1e-20)
    )
    }else {
      kl_bias <- torch::torch_sum(
    ...
    kl_weight <- torch::torch_sum(self$alpha * (torch::torch_log(
      self$weight_sigma_prior / self$weight_sigma) - 0.5 + torch::torch_log(
        self$alpha / self$alpha_prior + 1e-20) + (self$weight_sigma^2 +
        (self$weight_mean * z2 - self$weight_mean_prior)^2) / (
          2 * self$weight_sigma_prior^2)) + (1 - self$alpha) * torch::torch_log(
            (1 - self$alpha) / (1 - self$alpha_prior) + 1e-20))
...
#' @param prior_inclusion numeric scalar, prior inclusion probability for
...
#' @return A \code{torch::nn_module} object representing a convolutional
#' LBBNN layer.
#' The module has the following methods:
#'   - \code{forward(input, MPM = FALSE)}: Computes activation
#'   (using the LRT at training time) of a batch of inputs.
...
lbbnn_conv2d <- torch::nn_module(
  ...
  initialize = function(in_channels, out_channels, kernel_size, prior_inclusion,
    ...
    self$weight_mean <- torch::nn_parameter(torch_empty(out_channels,
                                                        in_channels, kernel[1],
                                                        kernel[2],
                                                        device = device))
    self$weight_rho <-  torch::nn_parameter(torch_empty(out_channels,
                                                        in_channels, kernel[1],
                                                        kernel[2],
                                                        device = device))
    self$weight_sigma <- torch::torch_empty(out_channels, in_channels,
                                            kernel[1], kernel[2],
                                            device = device)
    ...
    #inclusion variational parameters
    self$lambda_l <- torch::nn_parameter(torch_empty(out_channels, in_channels,
                                                     kernel[1], kernel[2],
                                                     device = device))
    self$alpha <- torch::torch_empty(out_channels, in_channels, kernel[1],
                                     kernel[2], device = device)

    #define priors.
    self$alpha_prior <- (torch::torch_zeros_like(
      self$alpha, device = self$device) + alpha_prior(prior_inclusion))

    #standard normal prior on the weights and biases
    self$weight_mean_prior <- torch::torch_zeros_like(self$weight_mean,
                                                      device = self$device)
    self$weight_sigma_prior <- (torch::torch_zeros_like(self$weight_sigma,
                                                        device = self$device)
                                + std_prior(standard_prior))
    self$bias_mean_prior <- torch::torch_zeros_like(self$bias_mean,
                                                    device = self$device)
    self$bias_sigma_prior <- (torch::torch_zeros_like(self$bias_sigma,
                                                      device = self$device)
                              + std_prior(standard_prior))

    #flow parameters
    if (self$flow) {
      self$q0_mean <- torch::nn_parameter(torch::torch_empty(out_channels,
                                                             device = device))
      self$q0_logvar <- torch::nn_parameter(torch::torch_empty(out_channels,
                                                               device = device))
      self$c1 <- torch::nn_parameter(torch::torch_empty(out_channels,
                                                        device = device))
  ...
  forward = function(input, MPM = FALSE) {
    ...
    self$weight_sigma <- torch::torch_log1p(torch_exp(self$weight_rho))
    self$bias_sigma <- torch::torch_log1p(torch_exp(self$bias_rho))
    z_k <- torch::torch_ones(self$out_channels, device = self$device) #if MF
    if (self$flow) {
      out <- self$sample_z()
      z_k <- out$z
    }
    ...
    }else {#only sample from mpm
      gamma <- (self$alpha$clone()$detach() > 0.5) * 1.
      w <- torch::torch_normal(self$weight_mean * z_k$view(c(-1, 1, 1, 1)),
                               self$weight_sigma)
      bias <- torch::torch_normal(self$bias_mean, self$bias_sigma)
      weight <- w * gamma
      activations <- torch::nnf_conv2d(input = input, weight = weight,
                                      bias = bias)
    }
  ...
  kl_div = function() {
    ...
    if (self$flow) {
      ...
      act <- act_mu + torch::torch_sqrt(act_var) * torch::torch_randn_like(
        act_var)
      mean_r <- self$b1$outer(act)$mean(-1)  # eq (9) from MNF paper
      log_var_r <- self$b2$outer(act)$mean(-1)  # eq (10) from MNF paper
      out2 <- self$sample_z()
    ...
    kl_bias <- torch::torch_sum(torch::torch_log(self$bias_sigma_prior
                                                 / self$bias_sigma)
                                - 0.5 + (self$bias_sigma ^ 2
                 + (self$bias_mean - self$bias_mean_prior) ^ 2)
                 / (2 * self$bias_sigma_prior ^ 2))

    kl_weight <- torch::torch_sum(self$alpha
                                  * (torch::torch_log(self$weight_sigma_prior
                                                      / self$weight_sigma)
                                  - 0.5 + torch::torch_log(self$alpha /
                                                      self$alpha_prior + 1e-20)
                                  + (self$weight_sigma ^ 2 +
                                       (self$weight_mean * z2$view(
                                         c(-1, 1, 1, 1))
                                        - self$weight_mean_prior) ^ 2) / (
                                        2 * self$weight_sigma_prior ^ 2))
                                  + (1 - self$alpha) * torch::torch_log(
                                    (1 - self$alpha) / (1 - self$alpha_prior)
                                    + 1e-20))
