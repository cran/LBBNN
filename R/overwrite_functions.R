...
#' @title Function that checks how many times inputs are included,
#' and from which layer. Used in summary function.
...
#' and L the total number of layers (including input and output),
...
#' \itemize{
#'   \item Number of times each input variable is included
#'         in the active paths from each layer
#'         (obtained from \code{get_input_inclusions()}).
#'   \item Average inclusion probabilities for each input variable from each layer,
#'.        including a final column showing the average across all layers.
#' }
#' @return A \code{data.frame} containing the above information.
...
#' @param object An object of class \code{lbbnn_net}.
#' @param type Only 'response' is implemented i.e. y_true - y_predicted.
#' @param ... further arguments passed to or from other methods.
...
#' @title Get model coefficients (local explanations) of \code{lbbnn_net} object
#' @description Given an input sample x_1,... x_j (with j the number of inputs,
#' the local explanation is found by considering active paths.
#' If relu activation functions are assumed, each path is a piecewise
#' linear function, so the contribution for x_j is just the sum of the weights
#' associated with the paths connecting x_j to the output.
#' The contributions are found by taking the gradient wrt x.
#' @param object an object of class \code{lbbnn_net}.
#' @param dataset Either a \code{torch::dataloader} object,
#' or a \code{torch::torch_tensor} object.
#' The former is assumed to be the same \code{torch::dataloader}
...
#' @param num_data integer, if no indices are chosen,
#' the first \code{num_data} of \code{dataset} are automatically used for explanations.
#' @param num_samples integer, how many samples to use for model averaging
#' when sampling the weights in the active paths.
#' @param ... further arguments passed to or from other methods.
#' @details
#' \itemize{
#'   \item If \code{num_data = 1}, confidence intervals are computed using
#'          model averaging over \code{num_samples} weight samples.
#'   \item If \code{num_data > 1}, confidence intervals are computed across
#'.         the mean explanations for each sample.
#'   \item The output is a data frame with row names as input variables
#'         (\code{x0}, \code{x1}, \code{x2}, ...) and columns giving mean
#'         and 95% confidence intervals for each variable.
#' }
#' @return A data frame with rows corresponding to input variables
...
coef.lbbnn_net <- function(object, dataset, inds = NULL, output_neuron = 1,
                           num_data = 1, num_samples = 10, ...) {
  if (output_neuron > object$sizes[length(object$sizes)])
  stop (paste("output_neuron =", output_neuron, "can not be greater than",
              object$sizes[length(object$sizes)]))
  if (is.null(inds)) {
  ...
  X <- dataset         # should be a tensor with shape (num_data,p)
  if (length(dim(X)) == 1) {
    X <- X$unsqueeze(dim = 1)
    } #reshape (p) to shape (1,p)
  if (dim(X)[length(dim(X))] != object$sizes[1])
  stop("the last index must have shape equal to p")
  }
  else {
  ...
  if (is.null(inds)) {
    if (dim(X)[1] < num_data)
    stop(paste("num_data = ", num_data,
               "can not be greater than the number of total data points,", dim(X)[1]))
    ...
    if (dim(X)[1] < length(inds))
    stop(paste("number of indecies =", length(inds),
      "can not be greater than the number of total data points,", dim(X)[1]))
    if (dim(X)[1] < max(inds))
    stop(paste("the largest index =", max(inds),
      "can not be greater than the number of total data points,", dim(X)[1]))
  ...
  if (num_data == 1) { #here we get the CI from the uncertainty around the one sample
    ...
    e <- expl$explanations
    e <- e[, , output_neuron] #get the explanation based on which output
...
#'@title Obtain predictions from the posterior of an \code{LBBNN model}
#'@description Draw from the posterior of a trained \code{lbbnn_net} object.
...
#'where \code{N} is the number of samples in \code{newdata},
...
predict.lbbnn_net <- function(object, newdata, mpm = FALSE, draws = 10,
  ...
  torch::with_no_grad({
    coro::loop(for (b in newdata){
      outputs <- torch::torch_zeros(draws, dim(b[[1]])[1],
...
#' @param data If local is chosen, one sample must be provided to obtain
...
plot.lbbnn_net <- function(x, type = c("global", "local"), data = NULL,
  ...
  if (d == "global") {
    ...
    if (is.null(data))
    stop("data must contain a sample to explain")
    plot_local_explanations_gradient(x, input_data = data,
                                     num_samples = num_samples,
                                     device = x$device, ...)
  }
